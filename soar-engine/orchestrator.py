...
"""
    ...
    def _generate_fallback_decision(self, findings: list, verified_logs: list, error_reason: str) -> dict:
        ...
        # preservation
        actions.append({
            "action_id": f"act-fallback-{act_id}",
            "phase": "preserve",
            "action_type": "preserve_logs",
            "target": {
                "type": "host",
                "value_masked": "WEB-PROD-FALLBACK"
            },
            "approval_mode": "AUTO",
            "status": "executed",
            "ttl_minutes": None,
            "rollback_plan": "No rollback needed",
            "playbook_source": "PB-FALLBACK",
            "rationale": "Preserve logs on local fallback",
            "risk_if_wrong": "low"
        })
        act_id += 1

        # Suggested block if verified
        ...
        if verified:
            actions.append({
                ...
                "target": {
                    "type": "ip",
                    "value_masked": source_ip
                },
                "approval_mode": "APPROVAL_REQUIRED",
                "status": "queued_for_approval",
                "ttl_minutes": 60,
                "rollback_plan": "Remove firewall block",
                "playbook_source": "PB-FALLBACK",
                "rationale": f"Suggest blocking IP {source_ip} due to verified activity",
