...
local function has_json_escape_breakout(text)
    ...
    -- A quote alone is normal in application logs. Only flag structural JSON
    -- breakout sequences (a closed object/array followed by a new quote) or
    -- explicit encoded quote escapes commonly used to smuggle that sequence.
...
function detect_threats(tag, timestamp, record)
    ...
    -- not operational keys such as "partitioner.ignore.keys".
